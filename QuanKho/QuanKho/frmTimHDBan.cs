...
namespace QuanKho
{
    public partial class frmTimHDBan : Form
    {
        ...
        private void dgvTKHoaDon_Click(object sender, EventArgs e)
        {
            ...
            {
                txtMaHDBan.Text = dgvTKHoaDon.CurrentRow.Cells["MaHDBan"].Value.ToString();
                txtMaNhanVien.Text = dgvTKHoaDon.CurrentRow.Cells["MaNhanVien"].Value.ToString();
                txtMaKhach.Text = dgvTKHoaDon.CurrentRow.Cells["MaNhanVien"].Value.ToString();
                txtMaKhach.Text = dgvTKHoaDon.CurrentRow.Cells["MaKhach"].Value.ToString();
                txtTongTien.Text = dgvTKHoaDon.CurrentRow.Cells["TongTien"].Value.ToString();

                // Lấy giá trị của cột NgayBan
                DateTime ngayBan = Convert.ToDateTime(dgvTKHoaDon.CurrentRow.Cells["NgayBan"].Value);
                // Lấy giá trị tháng từ cột NgayBan và hiển thị nó trong txtThang
                txtThang.Text = ngayBan.Month.ToString();
                txtNam.Text = ngayBan.Year.ToString();
            }
